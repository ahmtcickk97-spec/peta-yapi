...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${
      // Yazıların beyaz kalması için scroll olduğunda arka planı LACİVERT yaptık
      isScrolled ? 'bg-[#001F3F] shadow-2xl py-2' : 'bg-transparent py-4'
    }`}>
      <div className="container mx-auto px-4 md:px-8">
        <div className="flex justify-between items-center">
          
          {/* --- MARKA DEĞERİNİ ÖN PLANA ÇIKARAN LOGO --- */}
          <Link href="#anasayfa" className="flex flex-col items-start group no-underline font-sans leading-none">
            <div className="flex items-center">
              ...
              <div className="bg-[#001F3F] flex items-center px-2 py-1.5 md:px-3 md:py-2 rounded-[4px] h-[38px] md:h-[48px] border border-white/10">
                {/* P Sembolü */}
              ...
              {/* YAPI Yazısı - Sürekli Beyaz Kalıyor */}
              <span 
            ...
            {/* SLOGAN: Markayı boğmayan, ön plana çıkaran GÖK MAVİSİ dokunuşu */}
            <span className="mt-2 text-[8px] md:text-[10px] font-bold tracking-tight text-[#38BDF8]">
              Güvenle Dönüşüm, Sağlam Yarınlar
            </span>
          </Link>

          {/* SAĞ TARAF (Linkler lacivert arka planda beyaz kalacak) */}
          <div className="flex items-center space-x-4 md:space-x-10">
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <a 
                  key={link.name}
                  href={link.href}
                  className="text-[11px] font-black uppercase tracking-[0.2em] text-white hover:text-[#38BDF8] transition-all"
                >
                  {link.name}
                </a>
              ))}
            ...
            <a 
              ...
              className={`p-2 rounded-full transition-all hover:scale-110 ${
                isScrolled ? 'bg-white text-[#001F3F]' : 'bg-white text-[#001F3F]'
              }`}
            >
            ...
            <button onClick={() => setIsOpen(!isOpen)} className="md:hidden p-1 z-[120] relative text-white">
