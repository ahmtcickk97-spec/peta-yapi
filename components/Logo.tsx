export default function Logo({ isScrolled = false, isFooter = false }) {
  // Marka ismi (YAPI) scroll durumuna göre renk değiştirir
  const textColor = isFooter ? 'text-white' : (isScrolled ? 'text-black' : 'text-white');
  
  // Slogan artık marka isminden ayrışması için sabit Gök Mavisi (#38BDF8)
  const sloganColor = "text-[#38BDF8]"; 

  return (
    <div className="flex flex-col items-start font-sans select-none leading-none">
      {/* Üst Kısım: Bitişik Düzen */}
      ...
      {/* SLOGAN: Markayı boğmayan, ön plana çıkaran Gök Mavisi dokunuşu */}
